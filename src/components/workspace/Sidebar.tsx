...
export function Sidebar({ view, onSetView, counts, packs, onSelectPack, profile, onOpenProfile }: Props) {
  return (
    <aside className="hidden lg:flex flex-col bg-canvas-elevated border-r border-rule px-3 py-3.5">
      ...
      >
        <Icon name="plus" size={15} /> Add targets
      </button>

      <nav className="mt-1">
        <NavItem icon="overview" label="Overview" active={view === 'overview'} onClick={() => onSetView('overview')} />
        <NavItem icon="list" label="All targets" count={counts.total} active={view === 'all'} onClick={() => onSetView('all')} />
      </nav>

      <div className="mt-3">
        <div className="px-2.5 pt-2 pb-1 text-[11px] font-semibold uppercase tracking-[0.04em] text-ink-tertiary">Progress</div>
        <NavItem icon="send" label="Sent" count={counts.sent} active={view === 'sent'} onClick={() => onSetView('sent')} />
        <NavItem icon="clock" label="Awaiting reply" count={counts.awaiting} active={view === 'awaiting'} onClick={() => onSetView('awaiting')} />
        <NavItem icon="alert" label="Needs attention" count={counts.attention} active={view === 'attention'} onClick={() => onSetView('attention')} />
      </div>

      {packs.length > 0 && (
        <div className="mt-3">
          <div className="px-2.5 pt-2 pb-1 text-[11px] font-semibold uppercase tracking-[0.04em] text-ink-tertiary">Quick lists</div>
          {packs.map((p) => (
      ...
      >
        ...
        <span className="min-w-0">
          <span className="block text-[13px] font-medium truncate">{profile.fullName.trim() || 'Set your profile'}</span>
