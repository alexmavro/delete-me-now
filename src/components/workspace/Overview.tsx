  ...
  return (
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 mb-5">
        <Kpi label="Requests sent" value={stats.sent} icon="send" tone="bg-accent-soft text-accent" hint="in flight" />
        <Kpi label="Awaiting reply" value={stats.awaiting} icon="clock" tone="bg-honey-quiet text-honey" hint="statutory clock running" />
        <Kpi label="Resolved" value={stats.resolved} icon="check" tone="bg-positive-wash text-positive" hint="data confirmed deleted" />
        <Kpi label="Needs attention" value={stats.attention} icon="alert" tone="bg-critical-wash text-critical" hint="no reply or ready to escalate" />
      </div>

      {stats.total > 0 && (
        <div className="bg-canvas-elevated border border-rule rounded-[13px] p-5 shadow-sm mb-5">
          <div className="flex items-baseline justify-between mb-3">
            <b className="text-[14px] font-semibold">Erasure progress</b>
            <span className="text-[13px] text-ink-secondary">{stats.resolved} of {stats.total} resolved · {resolvedPct}%</span>
          </div>
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1.5 mt-3 text-[12.5px] text-ink-secondary">
            <Legend dot="bg-positive" label="Resolved" />
            <Legend dot="bg-accent" label="In flight" />
            <Legend dot="bg-critical" label="Needs attention" />
            <Legend dot="bg-canvas-sunken border border-rule-strong" label="Not started" />
          </div>
        </div>
      )}
    </>
